...
## What Happens During Vibecoding

Goal: show how the vibe coder's prompt gets combined with CodeSentinel's product rules before risk is evaluated.

```text
+----------------------+       +------------------------------+
| Company rules        |       | Role rules                   |
|                      |       |                              |
| - no PII exports     |       | - support can read tickets   |
| - no prod DB writes  |       | - support cannot dump users  |
| - flag secrets       |       | - approval for refunds       |
+----------+-----------+       +---------------+--------------+
           |                                   |
           +----------------+------------------+
                            |
                            v
+--------------------------------------------------------------+
|                    CODESENTINEL POLICY CONTEXT               |
|                                                              |
|  System prompt                                               |
|  + company global rules                                      |
|  + role-specific rules                                       |
|  + user/session metadata                                     |
|  + current vibe coder prompt                                 |
+------------------------------+-------------------------------+
                               |
                               v
+--------------------------------------------------------------+
|                         USER PROMPT                          |
|                                                              |
|  "Connect to the production database and export all users    |
|   to a CSV so I can email it to the auditor."                |
+------------------------------+-------------------------------+
                               |
                               v
+--------------------------------------------------------------+
...
|                                                              |
|  Check against rules                                         |
|  - production database access? yes                           |
|  - exports customer/user data? yes                           |
|  - sends data by email? yes                                  |
|                                                              |
|  Create RiskFlag                                             |
|  category: pii / policy                                      |
|  severity: critical                                          |
|  action: flag, warn, require approval, or block              |
+------------------------------+-------------------------------+
                               |
                               v
+--------------------------------------------------------------+
|                     CTO DASHBOARD VIEW                       |
|                                                              |
|  Session: refund dashboard                                   |
|  User: employee@company.com                                  |
|  Risk score: 92                                              |
|                                                              |
|  [Critical] PII / Policy                                     |
|  User attempted to export customer data from production.     |
|                                                              |
|  Safer version: Use an approved reporting workflow with      |
|  access controls and redacted fields.                        |
+--------------------------------------------------------------+
```

Description:

During vibecoding, CodeSentinel should evaluate the user's action using three sources of context:

1. The system prompt for CodeSentinel's analyst.
2. The company's product rules and role rules.
3. The vibe coder's current prompt, tool call, response, or code change.

The result is a `RiskFlag` with category, severity, explanation, suggested fix, and optional enforcement action. The dashboard then shows the CTO what happened and why it was risky.
